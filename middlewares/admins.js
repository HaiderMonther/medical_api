...
const fs = require("fs");
const { createAdminValidation } = require("../validations/admins");
...
async function usernameIsUnique(username) {
    const admin = await Admins.find({ username: username });
    if (admin.length > 0) {
        return false
    } else {
        return true
...
module.exports = { uploadImage: uploadImage, fileValidation: fileValidation, dataValidation: dataValidation }
